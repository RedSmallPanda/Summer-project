...
public interface ShowsRepository extends CrudRepository<Shows,Integer> {
    ...
    @Query("select distinct new com.sjtu.jpw.Repository.UtilClass.ShowTicket(t1.showId,t1.title,t1.info,t1.address,t1.rate) from Shows t1 join Ticket t2 on t1.showId=t2.showId where t1.city=:city and t1.type=:showtype and  t2.time between :starttime and :endtime ")
